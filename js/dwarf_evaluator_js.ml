...
let _ =
  ...
  context##.innerHTML := (Js.string (Sexp.to_string_hum (sexp_of_context_t Dwarf_evaluator.context)));
  let render _ =
    ...
    let output_html =
      try
        ...
        let result_loc = Dwarf_evaluator.eval_to_loc locexpr Dwarf_evaluator.context  in
